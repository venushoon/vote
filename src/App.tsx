...
function decodePollToken(token: string) {
...
export default function App() {
  ...
  function addOption() {
    setOptions((prev) => [...prev, { id: uuid(), label: `보기 ${prev.length + 1}`, votes: 0 }]);
  }
  ...
  function submitVote() {
    ...
    setBallots((prev) => ({
      ...prev,
      [key]: { ids: selected, at, name: anonymous ? undefined : voterName.trim() },
    }));
    setOptions((prev) =>
  ...
  function removeVoter(id: string) {
    ...
    if (!info) return;
    setOptions((prev) =>
      prev.map((o) => ({ ...o, votes: o.votes - (info.ids.includes(o.id) ? 1 : 0) }))
    );
    setBallots((prev) => {
      const next = { ...prev };
      delete next[id];
  ...
  function saveJSON() {
    ...
    download("vote-result.json", payload, "application/json");
    setSaveHint("JSON으로 저장했어요.");
  }
  function saveCSV() {
    const head = "option,votes\n";
    const rows = options.map((o) => `${o.label},${o.votes}`).join("\n");
    download("vote-summary.csv", head + rows, "text/csv");
  ...
  useEffect(() => {
    ...
    if (!token) return;
    try {
      const data = decodePollToken(token);
      if (data.title) setTitle(data.title);
      if (data.desc) setDesc(data.desc);
      if (data.voteLimit) setVoteLimit(data.voteLimit as 1 | 2);
      if (Array.isArray(data.options)) {
        setOptions(
          data.options.map((o: any) => ({ id: o.id || uuid(), label: o.label, votes: 0 }))
        );
      }
      if (typeof data.anonymous === "boolean") setAnonymous(data.anonymous);
      if (data.visibilityMode) setVisibilityMode(data.visibilityMode);
  ...
  function saveAndMakeShareLink() {
    ...
    const token = encodePollToken(payload);
    const { origin, pathname } = window.location;
    setShareLink(`${origin}${pathname}?p=${token}#student`);
    setSaveHint("설정 저장 & 공유링크 갱신 완료");
  }
    ...
    <div className="min-h-screen bg-gray-50 text-gray-900">
      <header className="sticky top-0 bg-white border-b">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className="h-10 w-10 rounded-2xl bg-indigo-600 text-white grid place-items-center font-bold">V</div>
            <div>
              {viewMode === "admin" ? (
                <input
                  className="text-xl md:text-2xl font-semibold bg-transparent border-b px-1 outline-none"
                  value={title}
              ...
              <div className="text-xs text-gray-500">1인 1·2표 / QR 공유 / 결과 공개 제어</div>
            </div>
          </div>

          {viewMode === "admin" && (
            <div className="flex items-center gap-1.5">
              <button onClick={saveJSON} className="px-2.5 py-1 text-xs rounded bg-indigo-600 text-white">JSON</button>
              <button onClick={saveCSV} className="px-2.5 py-1 text-xs rounded border">CSV</button>
              <button onClick={() => fileInputRef.current?.click()} className="px-2.5 py-1 text-xs rounded border">불러오기</button>
      ...
      {viewMode === "admin" ? (
        <AdminView
          ...
          deadlineAt={deadlineAt} setDeadlineAt={setDeadlineAt}
          totalVotes={totalVotes} graphData={graphData} isVisible={isVisible}
          addOption={addOption} setOptionLabel={setOptionLabel} removeOption={removeOption}
          clearAll={clearAll} recountVotes={recountVotes} removeVoter={removeVoter}
          saveHint={saveHint} setSaveHint={setSaveHint}
          effectiveStudentLink={effectiveStudentLink} saveAndMakeShareLink={saveAndMakeShareLink}
        />
      ) : (
        <StudentView
          title={title} desc={desc} options={options} voteLimit={voteLimit}
          anonymous={anonymous} visibilityMode={visibilityMode} deadlineAt={deadlineAt}
          isVisible={isVisible}
          voterName={voterName} setVoterName={setVoterName}
          selected={selected} setSelected={setSelected} submitVote={submitVote}
          totalVotes={totalVotes} graphData={graphData}
        />
      )}
    </div>
  );
...
type AdminProps = {
  ...
  voteLimit: 1 | 2; setVoteLimit: (v: 1 | 2) => void;
  options: Option[]; ballots: Ballots;
  anonymous: boolean; setAnonymous: (b: boolean) => void;
  visibilityMode: VisibilityMode; setVisibilityMode: (v: VisibilityMode) => void;
  deadlineAt: number | null; setDeadlineAt: (v: number | null) => void;
  totalVotes: number; graphData: { name: string; votes: number }[];
  isVisible: boolean;
  addOption: () => void; setOptionLabel: (id: string, label: string) => void; removeOption: (id: string) => void;
  clearAll: () => void; recountVotes: () => void; removeVoter: (id: string) => void;
  saveHint: string; setSaveHint: (s: string) => void;
  effectiveStudentLink: string; saveAndMakeShareLink: () => void;
};
...
function AdminView({
  ...
  totalVotes, graphData, isVisible, addOption, setOptionLabel, removeOption,
      ...
      <section className="md:col-span-2 space-y-6">
        ...
        <div className="bg-white rounded-2xl shadow p-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-1.5 whitespace-nowrap">
              <button
                onClick={saveAndMakeShareLink}
                className="px-2.5 py-1 text-xs rounded bg-indigo-600 text-white"
                title="설정 저장 및 공유링크 갱신"
              >
                저장
              </button>
              <button
                onClick={() => {
                  navigator.clipboard.writeText(effectiveStudentLink)
                    .then(() => setSaveHint("복사 완료"));
                }}
              ...
              <a
                href={effectiveStudentLink}
                className="px-2.5 py-1 text-xs rounded bg-emerald-600 text-white"
                target="_blank"
                rel="noreferrer"
                title="새 탭에서 학생 화면 열기"
              >
                열기
              </a>
            </div>
        ...
        <div className="bg-white rounded-2xl shadow p-4">
          <div className="flex items-center justify-between">
            ...
            <button onClick={addOption} className="px-2.5 py-1 text-xs rounded bg-indigo-600 text-white">
              추가
            </button>
          </div>
          <ul className="mt-3 space-y-2">
            {options.map((o) => (
              <li key={o.id} className="flex items-center gap-2">
                ...
                <button onClick={() => removeOption(o.id)} className="px-2 py-1 text-xs rounded border">
                  삭제
                </button>
              </li>
            ))}
          ...
          <div className="mt-3 flex items-center justify-between text-sm text-gray-500">
            <button onClick={clearAll} className="px-2.5 py-1 text-xs rounded border">전체 초기화</button>
            <button onClick={recountVotes} className="px-2.5 py-1 text-xs rounded border">표 재계산</button>
          </div>
        </div>
              ...
              {Object.entries(ballots).map(([id, info]) => (
                <li key={id} className="flex items-center justify-between py-2 text-sm">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={() => removeVoter(id)} className="px-2 py-1 text-xs rounded border">
                      삭제
                    </button>
                  </div>
                </li>
      ...
      <section className="md:col-span-3 space-y-6">
        ...
        <div className="bg-indigo-50 rounded-2xl border border-indigo-100 p-4">
          ...
          <ul className="list-disc pl-5 text-sm mt-2 space-y-1">
            ...
            <li>“마감 후 공개”로 설정하면 발표 전 비공개·마감 후 자동 공개가 됩니다.</li>
            <li>익명 모드에선 디바이스당 1회 투표로 제한됩니다.</li>
          </ul>
        </div>
      </section>
    </main>
    ...
    <main className="max-w-3xl mx-auto px-4 py-6 space-y-6">
      ...
      <div className="bg-white rounded-2xl shadow p-4">
        ...
        <div className="mt-4 flex items-center justify-between">
          ...
          <button onClick={onSubmit} className="px-4 py-2 rounded-xl bg-emerald-600 text-white hover:bg-emerald-700">
            제출
          </button>
        </div>
