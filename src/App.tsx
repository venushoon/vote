...
export default function App() {
  ...
  const [linkSnapshot, setLinkSnapshot] = useState<string>(""); // s= 토큰(스냅샷)

  // ---- 로드 순서: URL 스냅샷 > localStorage ----
  useEffect(() => {
    ...
    if (s) {
      try {
        ...
        setLinkSnapshot(s); // 현재 링크가 가진 스냅샷 유지
        return;
      } catch (e) {
  ...
  function applySnapshot(snap: Snapshot, opts?: { resetVotes?: boolean }) {
    ...
    setOptions(prev => {
      const base = snap.options.map(x => {
        const prevVotes = opts?.resetVotes ? 0 : (prev.find(p => p.id === x.id)?.votes ?? 0);
        return { id: x.id, label: x.label, votes: prevVotes };
      });
      return base;
    });
  ...
  function saveToLocalNow() {
    ...
    // 2) URL 스냅샷 갱신
    const snapStr = JSON.stringify(buildSnapshot());
    const token = b64url.encode(snapStr);
  ...
  function getStudentKey(): string {
    if (anonymous) {
      const token = localStorage.getItem("vote_device_token") || (() => {
        const t = uuid();
  ...
  function resetAllToDefaults() {
    ...
    setVoterName(""); setSelected([]);
    localStorage.removeItem(STORAGE_KEY);

    // 기본 상태로 새 스냅샷 만들어 링크/QR 갱신
    ...
    reader.onload = ev => {
      try {
        ...
        const token = b64url.encode(JSON.stringify({
          ...
          options: (data.options ?? options).map((o: any) => ({ id: o.id, label: o.label })),
          anonymous: data.anonymous ?? anonymous,
          visibilityMode: (data.visibilityMode ?? visibilityMode) as Visibility,
          deadlineAt: data.deadlineAt ?? deadlineAt,
  ...
  const copyStudentLink = () => navigator.clipboard.writeText(studentLink).then(() => setSaveHint("학생용 링크를 복사했어요."));
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-white to-gray-50 text-gray-900">
      ...
      <header className="sticky top-0 z-10 bg-white/90 backdrop-blur border-b">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              {viewMode === "admin" ? (
                <input className="text-xl md:text-2xl font-semibold bg-transparent border-b border-transparent focus:border-indigo-400 outline-none px-1 rounded"
                  value={title} onChange={e => setTitle(e.target.value)} aria-label="제목" />
              ) : (
      ...
      {viewMode === "admin" ? (
        <AdminView
          {...{
            ...
            removeVoter: (id: string) => {
              const info = ballots[id]; if (!info) return;
        ...
        <StudentView
          {...{
            desc, options, voteLimit, anonymous, visibilityMode, deadlineAt, isVisible,
...
function AdminView(props: any) {
  const {
    desc, setDesc, voteLimit, setVoteLimit, options, ballots,
    anonymous, setAnonymous, visibilityMode, setVisibilityMode,
    deadlineAt, setDeadlineAt, totalVotes, graphData, isVisible,
    addOption, setOptionLabel, removeOption, recountVotes,
  ...
  return (
    <main className="max-w-6xl mx-auto px-4 py-6 grid lg:grid-cols-5 gap-6">
      ...
      <section className="lg:col-span-2 space-y-6">
          ...
          <div className="mt-4 grid grid-cols-1 gap-3">
            <div className="flex flex-wrap items-center gap-3">
              <div className="flex items-center gap-2">
                ...
                <select value={voteLimit} onChange={e => setVoteLimit(Number(e.target.value) as VoteLimit)} className="border rounded-lg px-2 py-1">
              ...
              <div className="flex items-center gap-2">
                ...
                {visibilityMode === "deadline" && (
                  <input
                    type="datetime-local" className="border rounded-lg px-2 py-1"
            ...
            <div className="flex flex-wrap items-center gap-3">
              <div className="flex items-center gap-2">
                ...
                <input type="number" min={0} className="w-24 border rounded-lg px-2 py-1"
                  value={expectedVoters} onChange={e => setExpectedVoters(Math.max(0, Number(e.target.value||0)))} />
        ...
        <div className="bg-white rounded-2xl shadow p-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={saveToLocalNow} className="px-3 py-1.5 text-sm rounded-lg bg-white border hover:bg-gray-50" title="현재 설정을 저장하고 QR/링크를 갱신합니다">저장</button>
            </div>
          </div>
        ...
        <div className="bg-white rounded-2xl shadow p-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={copyStudentLink} className="px-3 py-1.5 text-sm rounded-lg bg-white border hover:bg-gray-50">복사</button>
        ...
        {/* 투표자 목록 */}
        <div className="bg-white rounded-2xl shadow p-4">
          <div className="flex items-center justify-between"><h2 className="font-semibold">투표자 목록</h2></div>
          {Object.keys(ballots).length === 0 ? (
            ...
            <ul className="mt-2 max-h-48 overflow-auto divide-y">
              {Object.entries(ballots).map(([id, info]: [string, Ballot & { name?: string }]) => (
                <li key={id} className="flex items-center justify-between py-2 text-sm">
                  <div>
                    <div className="font-medium">{(info as any).name || id}</div>
                    <div className="text-xs text-gray-500">{new Date(info.at).toLocaleString()}</div>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="text-xs text-gray-500">
                      {info.ids.map(x => options.find(o => o.id === x)?.label).filter(Boolean).join(", ")}
                    </div>
            ...
            {isVisible ? (
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={graphData} margin={{ top:20, right:20, bottom:20, left:0 }}>
                  ...
                  <Bar dataKey="votes" fill="url(#barGrad)" radius={[10,10,0,0]} barSize={36}
...
function StudentView(props: any) {
  ...
  return (
    <main className="max-w-3xl mx-auto px-4 py-6 space-y-6">
      ...
      <div className="bg-white rounded-2xl shadow p-4">
        {isClosed && <div className="mb-3 p-3 rounded-xl bg-rose-50 text-rose-700 text-sm border border-rose-100">투표가 마감되어 더 이상 제출할 수 없습니다.</div>}

        {!anonymous && (
          <div className="mb-3">
            <label className="text-sm text-gray-500">이름/번호</label>
            <input value={voterName} onChange={(e:any)=>setVoterName(e.target.value)} placeholder="이름 또는 번호"
              className="mt-1 w-full border rounded-lg px-3 py-2" disabled={isClosed}/>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          {options.map((o: Option) => {
            ...
            return (
              <button key={o.id}
                onClick={() => setSelected((prev:string[]) => {
          ...
          {isVisible ? (
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={graphData} margin={{ top:20, right:20, bottom:20, left:0 }}>
                ...
                <YAxis allowDecimals={false}/>
                <Tooltip formatter={(v:any)=>`${v} 표`}/>
                <Bar dataKey="votes" fill="url(#barGradS)" radius={[10,10,0,0]} barSize={32}
                     isAnimationActive animationDuration={700} animationEasing="ease-out">
                  <LabelList dataKey="votes" position="top"/>
                </Bar>
              </BarChart>
