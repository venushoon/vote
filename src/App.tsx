      ...
      <section className="md:col-span-3 space-y-6">
        ...
        <div className="bg-indigo-50 rounded-2xl border border-indigo-100 p-4">
          ...
          <ul className="list-disc pl-5 text-sm mt-2 space-y-1">
            <li>화면 우상단 메뉴로 저장/불러오기 가능합니다 (JSON/CSV).</li>
            <li>"결과 공개"를 <b>마감 후 공개</b>로 두고 마감 시각을 설정하면, 발표 전 비공개·마감 후 자동 공개가 됩니다.</li>
...
function StudentView(props: any) {
  const { desc, options, voteLimit, anonymous, visibilityMode, deadlineAt, isVisible, voterName, setVoterName, selected, setSelected, toggleSelect, submitVote, totalVotes, graphData, isClosed } = props;
  ...
  return (
    <main className="max-w-3xl mx-auto px-4 py-6 space-y-6">
      ...
      <div className="bg-white rounded-2xl shadow p-4">
        {isClosed && (
          <div className="mb-3 p-3 rounded-xl bg-rose-50 text-rose-700 text-sm border border-rose-100">투표가 마감되어 더 이상 제출할 수 없습니다.</div>
        )}

        {!anonymous && (
          <div className="mb-3">
            <label className="text-sm text-gray-500">이름/번호</label>
            <input value={voterName} onChange={(e:any) => setVoterName(e.target.value)} placeholder="이름 또는 번호" className="mt-1 w-full border rounded-lg px-3 py-2" disabled={isClosed} />
          </div>
        )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          {options.map((o:any) => {
            ...
            return (
              <button key={o.id} onClick={() => setSelected((prev:string[]) => {
                if (isClosed) return prev;
                if (prev.includes(o.id)) return prev.filter((x:string) => x !== o.id);
                if (prev.length >= voteLimit) return prev;
                return [...prev, o.id];
              })} className={`text-left p-3 rounded-xl border transition ${checked ? "bg-indigo-50 border-indigo-300" : "bg-white hover:bg-gray-50"} ${disabled ? "opacity-50 cursor-not-allowed" : ""}`} disabled={disabled}>
    ...
    <main className="max-w-3xl mx-auto px-4 py-6 space-y-6">
      ...
      <div className="bg-white rounded-2xl shadow p-4">
        {!anonymous && (
          <div className="mb-3">
            <label className="text-sm text-gray-500">이름/번호</label>
            <input value={voterName} onChange={(e:any) => setVoterName(e.target.value)} placeholder="이름 또는 번호" className="mt-1 w-full border rounded-lg px-3 py-2" />
          </div>
        )}
        ...
        <div className="mt-4 flex items-center justify-between">
          <a href="#admin" className="text-xs text-gray-400 underline">관리자 화면</a>
          <button onClick={onSubmit} className="px-4 py-2 rounded-xl bg-emerald-600 text-white hover:bg-emerald-700">제출</button>
        </div>
