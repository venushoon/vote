...
// ✅ 교실 투표 프로그램 (QR 학생용 화면, 익명/실명 토글, 결과 공개 제어, 마감 후 자동공개)
// - 관리자(교사) 화면과 학생 화면을 URL 해시로 분리: #admin / #student
// - QR코드로 학생 화면 링크 제공, 링크 복사 기능
// - 익명/실명 모드 토글
// - 결과 숨김/공개 스위치 + "발표 전 비공개, 마감 후 공개" 옵션
// - 1인 1표·2표 지원, 실시간 그래프, JSON/CSV 저장·불러오기

// ▼ 유틸: 간단 UUID
function uuid() {
  // 브라우저 환경 가정: crypto.randomUUID 존재 시 사용
  // (폴백: 임시 랜덤)
  return (window.crypto?.randomUUID?.() || `id-${Math.random().toString(36).slice(2)}-${Date.now()}`);
}

// ▼ 현재 뷰 모드: 해시 기반 (#student 면 학생화면, 기본은 관리자)
function getViewMode() {
  return (location.hash === "#student") ? "student" : "admin";
}

export default function ClassroomVotingApp() {
  ...
  const [voteLimit, setVoteLimit] = useState(1); // 1 또는 2
  const [options, setOptions] = useState([
    { id: uuid(), label: "보기 1", votes: 0 },
    { id: uuid(), label: "보기 2", votes: 0 },
  ]);

  // 투표 기록: key(투표자키) -> { ids: string[], at: number, name?: string }
  const [ballots, setBallots] = useState<Record<string, { ids: string[]; at: number; name?: string }>>({});

  // 모드/공개 설정
  const [anonymous, setAnonymous] = useState(false); // true=익명(학생화면에서 이름 입력 숨김)
  const [visibilityMode, setVisibilityMode] = useState<"always" | "hidden" | "deadline">("always");
  const [deadlineAt, setDeadlineAt] = useState<number | null>(null);

  // ⏱️ 투표 인원 설정 & 자동 마감
  const [expectedVoters, setExpectedVoters] = useState<number>(0); // 0=미설정
  const [manualClosed, setManualClosed] = useState(false); // 수동 마감/재개

  // 학생 측 입력 상태
  const [voterName, setVoterName] = useState(""); // 실명 모드에서 사용
  ...
  useEffect(() => {
    ...
    } catch (e) { console.warn("로컬 데이터 불러오기 실패", e); }
  }, []);

  useEffect(() => {
    const payload = JSON.stringify({ title, desc, voteLimit, options, ballots, anonymous, visibilityMode, deadlineAt, expectedVoters, manualClosed });
  ...
  // ====== 옵션 편집/유틸 ======
  function setOptionLabel(id: string, label: string) { setOptions(prev => prev.map(o => (o.id === id ? { ...o, label } : o))); }
  function addOption() { const label = `보기 ${options.length + 1}`; setOptions(prev => [...prev, { id: uuid(), label, votes: 0 }]); }
  function removeOption(id: string) {
    setOptions(prev => prev.filter(o => o.id !== id));
    setBallots(prev => {
      const next: typeof prev = {};
      Object.entries(prev).forEach(([k, info]) => { next[k] = { ...info, ids: info.ids.filter(x => x !== id) }; });
  ...
  function getStudentKey() {
    if (anonymous) {
      ...
      if (!token) { token = uuid(); localStorage.setItem("vote_device_token", token); }
      return token;
    }
  ...
  function submitVote() {
    ...
    setBallots(prev => ({ ...prev, [key]: { ids: selected, at: nowMs, name: anonymous ? undefined : voterName.trim() } }));
    setOptions(prev => prev.map(o => ({ ...o, votes: o.votes + (selected.includes(o.id) ? 1 : 0) })));
    setVoterName("");
    setSelected([]);
  }
  ...
  function closeNow() { setManualClosed(true); }
  function reopen() { setManualClosed(false); }
  ...
  function saveJSON() {
    const payload = JSON.stringify({ title, desc, voteLimit, options, ballots, anonymous, visibilityMode, deadlineAt, expectedVoters, manualClosed }, null, 2);
  ...
  function escapeCSV(s: string) { if (s == null) return ""; const needs = /[",\n]/.test(s); const out = String(s).replace(/"/g, '""'); return needs ? '"' + out + '"' : out; }
  ...
  function loadFromFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = ev => {
      ...
      } catch { alert("불러오기에 실패했어요. JSON 형식을 확인하세요."); }
    };
  ...
  // ====== 공유 링크/QR ======
  const studentLink = useMemo(() => { const url = new URL(location.href); url.hash = "#student"; return url.toString(); }, [location.href]);
  function copyStudentLink() { navigator.clipboard.writeText(studentLink).then(() => setSaveHint("학생용 링크를 복사했어요.")); }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-white to-gray-50 text-gray-900">
      ...
      <header className="sticky top-0 z-10 bg-white/90 backdrop-blur border-b">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              {viewMode === "admin" ? (
                <input className="text-xl md:text-2xl font-semibold bg-transparent border-b border-transparent focus:border-indigo-400 outline-none px-1 rounded" value={title} onChange={e => setTitle(e.target.value)} aria-label="제목" />
              ) : (
          ...
          <div className="border-t bg-gradient-to-r from-indigo-50 to-purple-50">
            <div className="max-w-6xl mx-auto px-4 py-2 flex items-center gap-3 text-sm">
              ...
              <div className="flex-1 h-2 bg-white/60 rounded-full overflow-hidden">
                <div className="h-full bg-indigo-500" style={{ width: `${Math.min(100, expectedVoters? (votedCount/expectedVoters)*100 : 0)}%` }} />
              </div>
            </div>
          </div>
        )}
      ...
      {viewMode === "admin" ? (
        <AdminView {...{ desc, setDesc, voteLimit, setVoteLimit, options, ballots, anonymous, setAnonymous, visibilityMode, setVisibilityMode, deadlineAt, setDeadlineAt, totalVotes, graphData, isVisible, addOption, setOptionLabel, removeOption, clearAll, recountVotes, removeVoter, saveHint, studentLink, copyStudentLink, expectedVoters, setExpectedVoters, isClosed, closeNow, reopen }} />
      ) : (
        <StudentView {...{ desc, options, voteLimit, anonymous, visibilityMode, deadlineAt, isVisible, voterName, setVoterName, selected, setSelected, toggleSelect, submitVote, totalVotes, graphData, isClosed }} />
      )}
  ...
  // 모드/공개 설정
  const [anonymous, setAnonymous] = useState(false); // true=익명(학생화면에서 이름 입력 숨김)
  // 결과 공개 모드: always(항상), hidden(항상 숨김), deadline(마감 전 숨김/마감 후 공개)
  const [visibilityMode, setVisibilityMode] = useState<"always" | "hidden" | "deadline">("always");
  const [deadlineAt, setDeadlineAt] = useState<number | null>(null);
  ...
  useEffect(() => {
    const payload = JSON.stringify({ title, desc, voteLimit, options, ballots, anonymous, visibilityMode, deadlineAt });
    localStorage.setItem(STORAGE_KEY, payload);
  }, [title, desc, voteLimit, options, ballots, anonymous, visibilityMode, deadlineAt]);
  ...
  function toggleSelect(id: string) {
    setSelected(prev => {
      if (prev.includes(id)) return prev.filter(x => x !== id);
      if (prev.length >= voteLimit) return prev; // 제한
      return [...prev, id];
    });
  }
  ...
  function submitVote() {
    ...
    const now = Date.now();
    setBallots(prev => ({ ...prev, [key]: { ids: selected, at: now, name: anonymous ? undefined : voterName.trim() } }));
    setOptions(prev => prev.map(o => ({ ...o, votes: o.votes + (selected.includes(o.id) ? 1 : 0) })));
  ...
  function removeVoter(id: string) {
    ...
    setBallots(prev => { const next = { ...prev }; delete next[id]; return next; });
  }
  ...
  function saveJSON() {
    const payload = JSON.stringify({ title, desc, voteLimit, options, ballots, anonymous, visibilityMode, deadlineAt }, null, 2);
    const stamp = new Date().toISOString().replace(/[:.]/g, "-");
    download(`vote-result-${stamp}.json`, payload, "application/json");
    setSaveHint("JSON으로 저장했어요.");
  }
...
");
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-900">
        ...
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-xs text-gray-500">1인 1·2표 / QR 학생화면 / 결과공개 제어 / 저장</div>
            </div>
          </div>
      ...
      {viewMode === "admin" ? (
        <AdminView
          {...{ title, setTitle, desc, setDesc, voteLimit, setVoteLimit, options, setOptions, ballots, setBallots, anonymous, setAnonymous, visibilityMode, setVisibilityMode, deadlineAt, setDeadlineAt, totalVotes, graphData, isVisible, addOption, setOptionLabel, removeOption, clearAll, recountVotes, removeVoter, saveHint, setSaveHint, studentLink, copyStudentLink, voteLimitState: voteLimit }}
        />
      ) : (
        <StudentView
          {...{ title, desc, options, voteLimit, anonymous, visibilityMode, deadlineAt, isVisible, voterName, setVoterName, selected, setSelected, toggleSelect, submitVote, totalVotes, graphData }}
        />
      )}
...
function AdminView(props: any) {
  const {
    ...
    addOption, setOptionLabel, removeOption,
    clearAll, recountVotes, removeVoter,
    ...
    <main className="max-w-6xl mx-auto px-4 py-6 grid lg:grid-cols-5 gap-6">
      ...
      <section className="lg:col-span-2 space-y-6">
        ...
        <div className="bg-white rounded-2xl shadow p-4">
          ...
          <textarea value={desc} onChange={e => setDesc(e.target.value)} className="w-full mt-2 p-3 border rounded-xl outline-none focus:ring-2 focus:ring-indigo-300" rows={3} placeholder="투표 목적/안내를 적어주세요." />

          <div className="mt-4 grid grid-cols-1 gap-3">
            <div className="flex flex-wrap items-center gap-3">
              <div className="flex items-center gap-2">
                <span className="text-sm text-gray-500">투표 방식</span>
                <select value={voteLimit} onChange={e => setVoteLimit(Number(e.target.value))} className="border rounded-lg px-2 py-1">
              ...
              <div className="flex items-center gap-2">
                ...
                {visibilityMode === "deadline" && (
                  <input type="datetime-local" className="border rounded-lg px-2 py-1" value={deadlineAt ? new Date(deadlineAt).toISOString().slice(0,16) : ""} onChange={e => setDeadlineAt(e.target.value ? new Date(e.target.value).getTime() : null)} />
                )}
              </div>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3">
              <div className="flex items-center gap-2">
                ...
                <input type="number" min={0} className="w-24 border rounded-lg px-2 py-1" value={expectedVoters} onChange={e => setExpectedVoters(Math.max(0, Number(e.target.value||0)))} />
                <span className="text-xs text-gray-500">0이면 자동 마감 없음</span>
              </div>
              <div className="flex items-center gap-2">
                {!isClosed ? (
                  <button onClick={closeNow} className="px-3 py-1.5 text-sm rounded-lg bg-rose-600 text-white hover:bg-rose-700 shadow">지금 마감</button>
                ) : (
                  <button onClick={reopen} className="px-3 py-1.5 text-sm rounded-lg bg-emerald-600 text-white hover:bg-emerald-700 shadow">재개</button>
                )}
              </div>
            </div>
        ...
        <div className="bg-white rounded-2xl shadow p-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <button onClick={copyStudentLink} className="px-3 py-1.5 text-sm rounded-lg bg-white border hover:bg-gray-50">링크 복사</button>
              <a href="#student" className="px-3 py-1.5 text-sm rounded-lg bg-indigo-600 text-white hover:bg-indigo-700">바로 열기</a>
            </div>
          </div>
          <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-3 items-center">
            ...
            <div className="text-sm text-gray-600 break-all leading-relaxed">
              {studentLink}
              <p className="mt-2 text-xs text-gray-500">QR을 화면에 띄우거나 링크를 메시지/알림장으로 공유하세요.</p>
            </div>
          </div>
        </div>
          ...
          <ul className="mt-3 space-y-2">
            {options.map((o: any) => (
              <li key={o.id} className="flex items-center gap-2">
                <input className="flex-1 border rounded-lg px-2 py-1" value={o.label} onChange={e => setOptionLabel(o.id, e.target.value)} />
            ...
            <ul className="mt-2 max-h-48 overflow-auto divide-y">
              {Object.entries(ballots).map(([id, info]: any) => (
                <li key={id} className="flex items-center justify-between py-2 text-sm">
                  ...
                  <div className="flex items-center gap-2">
                    <div className="text-xs text-gray-500">{info.ids.map((x: string) => options.find((o: any) => o.id === x)?.label).filter(Boolean).join(", ")}</div>
      ...
      <section className="md:col-span-2 space-y-6">
        ...
        <div className="bg-white rounded-2xl shadow p-4">
          ...
          <div className="mt-4 grid grid-cols-1 gap-3">
            ...
            <div className="flex items-center justify-between text-sm text-gray-500">
              <div>총 투표수: <span className="font-semibold">{totalVotes}</span></div>
              <div>{saveHint}</div>
            </div>
          </div>
        </div>
